...
import "./style.css"
import { pegarValores, loadTasks, storage, choseData} from './app.js';
import formValid from './formValidation.js';
import { valid, warning, titleValidation, validTitle, warningTitle} from './formValidation.js';
import { choseTask, delTask } from './deletTask.js';
import listTask, { createDiv, projectClick, saveToLocalStorage } from './navTask.js';
  ...
  input.forEach((item) =>{
      ...
      if(item.id === 'dueDate'){
        ...
        if(valid === true){
          ...
          date.push(`${vality.getDate() + 1} / ${vality.getMonth() + 1} / ${vality.getFullYear()}`)
          task.dueDate = date
